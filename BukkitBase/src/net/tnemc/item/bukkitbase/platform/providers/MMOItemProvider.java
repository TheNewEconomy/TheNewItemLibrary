...
public class MMOItemProvider implements ItemProvider<ItemStack> {
  ...
  public boolean similar(final AbstractItemStack<? extends ItemStack> original, final ItemStack compare) {
    ...
    final String id = MMOItems.getID(compare);
    if(id == null) {
      return false;
    }

    return id.equals(original.providerItemID());
  }
  ...
  public ItemStack locale(final AbstractItemStack<? extends ItemStack> original, final int amount) {
    ...
    final String[] splitValue = original.providerItemID().split(":");

    return MMOItems.plugin.getItem(splitValue[0], original.providerItemID());
  }
