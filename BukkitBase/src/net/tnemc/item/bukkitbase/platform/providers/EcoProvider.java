 ...
 * @since 0.2.0.0
 */
public class EcoProvider implements ItemProvider<ItemStack> {
  ...
  public boolean similar(final AbstractItemStack<? extends ItemStack> original, final ItemStack compare) {
    ...
    final List<EcoItem> items = EcoItemFinder.INSTANCE.find(compare);
    if(items.isEmpty()) {
      return false;
    }

    final EcoItem item = items.get(0);

    return item.getID().equals(original.providerItemID());
  }
  ...
  public ItemStack locale(final AbstractItemStack<? extends ItemStack> original, final int amount) {
    ...
    final EcoItem ecoItem = EcoItems.INSTANCE.getByID(original.providerItemID());
    if(ecoItem == null) {

      return null;
    }

    final ItemStack stack = ecoItem.getItemStack();
