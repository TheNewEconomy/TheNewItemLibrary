 ...
 * @since 0.2.0.0
 */
public class SlimefunProvider implements ItemProvider<ItemStack> {
  ...
  public boolean appliesTo(final AbstractItemStack<? extends ItemStack> serialized, final ItemStack item) {
    ...
    final SlimefunItem stack = SlimefunItem.getByItem(item);
    if(stack == null) {
      return false;
    }

    serialized.setItemProvider(identifier());
    serialized.setProviderItemID(stack.getId());
  ...
  public boolean similar(final AbstractItemStack<? extends ItemStack> original, final ItemStack compare) {
    ...
    final SlimefunItem compareStack = SlimefunItem.getByItem(compare);
    if(compareStack == null) {

      return false;
    }

    return original.providerItemID().equals(compareStack.getId());
  }
  ...
  public ItemStack locale(final AbstractItemStack<? extends ItemStack> original, final int amount) {
    ...
    final SlimefunItem originalStack = SlimefunItem.getById(original.providerItemID());
    if(originalStack == null) {

      return null;
    }

    final ItemStack stack = originalStack.getItem();
