...
/**
 * PaperCanBreakComponent
 *
 * @author creatorfromhell
 * @since 0.2.0.0
 */
public class PaperCanBreakComponent extends CanBreakComponent<PaperItemStack, ItemStack> {

  public PaperCanBreakComponent() {

  }

  public PaperCanBreakComponent(final List<net.tnemc.item.component.helper.BlockPredicate> predicates) {
  ...
  public ItemStack apply(final PaperItemStack serialized, final ItemStack item) {
    ...
    final Optional<PaperCanBreakComponent> componentOptional = serialized.component(identifier());
    if(componentOptional.isEmpty()) {
      return item;
    }

    item.setData(DataComponentTypes.CAN_BREAK, ItemAdventurePredicate.itemAdventurePredicate(
  ...
  public PaperItemStack serialize(final ItemStack item, final PaperItemStack serialized) {
    ...
    final ItemAdventurePredicate adventurePredicate = item.getData(DataComponentTypes.CAN_BREAK);
    if(adventurePredicate == null) {
      return serialized;
    }

    final PaperCanBreakComponent component = (serialized.paperComponent(identifier()) instanceof final CanBreakComponent<?, ?> getComponent)?
                                             (PaperCanBreakComponent)getComponent : new PaperCanBreakComponent();
