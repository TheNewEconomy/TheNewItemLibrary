 ...
 * @since 0.1.7.7
 */
public abstract class ItemProfile<I extends AbstractItemStack<T>, T> implements ItemCheck<T>, ItemApplicator<I, T>, ItemDeserializer<I, T> {
  ...
  public boolean check(AbstractItemStack<T> original, AbstractItemStack<T> check) {
    if(original.profile().isPresent()) {

      if(check.profile().isEmpty()) return false;

      return original.profile().get().equals(check.profile().get());
    }
    return check.profile().isPresent();
  }
}