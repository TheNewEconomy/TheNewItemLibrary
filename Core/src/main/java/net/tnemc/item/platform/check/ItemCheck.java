...
/**
 * Represents a check to be performed on an item. Implementations should provide logic to determine
 * if the check applies for a specific version, as well as perform the actual check on the item
 * stack.
 *
...
public interface ItemCheck<T> extends Identifiable {
  ...
  /**
   * Determines if a given check should be applied to an original item stack for a specific
   * version.
   *
