...
public class VanillaProvider implements ItemProvider<ItemStack> {
  ...
  public boolean similar(final AbstractItemStack<? extends ItemStack> original, final ItemStack compare) {
    ...
    final SpongeItemStack compareStack = new SpongeItemStack().of(compare);

    return SpongeItemPlatform.instance().check((SpongeItemStack)original, compareStack);
  }
  ...
  public ItemStack locale(final AbstractItemStack<? extends ItemStack> original, final int amount) {
    ...
    if(original instanceof final SpongeItemStack sponge) {

      if(!sponge.isDirty()) {

        return sponge.cacheLocale();
      }

      final ItemType type = ItemTypes.registry().value(fromString(sponge.material()));

      ItemStack stack = ItemStack.builder().itemType(type).quantity(amount).build();

      stack = SpongeItemPlatform.instance().apply(sponge, stack);

      sponge.updateCache(stack);
      sponge.resetDirty();
